...
let
  cfg = config.lama-corp.services.xserver.windowManager.bar;
  ...
  batteryModule = types.submodule {
    options = {
      device = mkOption {
        default = "BAT0";
        description = "Battery to be monitored by the bar engine.";
      };
      fullAt = mkOption {
        default = 98;
        description = "In case the battery never reports 100% charge.";
      };
    };
  };
in
{
  options = {
    lama-corp.services.xserver.windowManager.bar = {
