...
if [[ -n "${TMUX}" ]]; then
	POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(dir vcs status vi_mode command_execution_time background_jobs root_indicator ssh)
else
	POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(dir vcs status vi_mode command_execution_time background_jobs context root_indicator ssh time)
fi
