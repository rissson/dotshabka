# Secrets like hue key
source ~/.envrc

# Autmatically set the title to the last command
DISABLE_AUTO_TITLE="true"

function set_win_title() {
    title="ZSH - $(pwd) - $(history | tail -n 1)"
...
function bin() {
	echo $1
	curl -d name=Diego --data-urlencode text@$1 https://bin.lama-corp.space/api/create 
}

function bin-dl() {
	 curl https://bin.lama-corp.space/view/raw/$1
}

function timer() {
	i3-msg sticky enable, floating enable
	termdown "$1"

	if [ $2 ]; then
		notify-send "$2"
	else
		notify-send "Fin du timer"
	fi
	i3-msg sticky disable, floating disable
}
...
export CODE_PATH="@home_path@/prog"
