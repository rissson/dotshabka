...
let
  ...
  hashedServices = with builtins;
    (mapAttrs' (name: service:
      nameValuePair "${name}_${config.networking.hostName}_${
        unsafeDiscardStringContext
        (substring 0 12 (hashString "md5" (toJSON service)))
  ...
  configureScript = pkgs.writeShellScriptBin "gitlab-runner-configure"
    (if (cfg.configFile != null) then ''
      ...
      chown -R $(stat -c '%U:%G' $(readlink -f $HOME)) $(dirname ${configPath})
    '' else ''
...
in {
  options.services.gitlab-runner = {
    ...
    configFile = mkOption {
      ...
      description = ''
        ...
        Warning: Do NOT use this option unless forced,
        you should use <option>services</option> instead.
      ...
      type = types.attrsOf (types.submodule {
        options = {
          registrationConfigFile = mkOption {
            type = types.path;
            description = ''
              File with environment variables used for gitlab-runner registration.
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.gitlab-runner = {
      ...
      path = with pkgs;
        [ bash gawk jq moreutils remarshal utillinux cfg.package.bin ]
        ++ cfg.extraPackages;
