...
in {
  systemd.services.scoreboard-seedbox-cri =
    mkIf (builtins.pathExists /srv/http/scoreboard-seedbox-cri) {
      enable = true;
        ...
        djangoEnv = nixpkgs.poetry2nix.mkPoetryEnv {
          ...
          overrides = nixpkgs.poetry2nix.overrides.withDefaults (self: super: {
            ...
            social-auth-backend-epita =
              super.social-auth-backend-epita.overridePythonAttrs (old: {
                postPatch = ''
      ...
      environment = { DJANGO_IGNORE_MISSING_ENV = "1"; };
    };
  ...
  services.nginx.virtualHosts."scoreboard-seedbox-cri.risson.space" = {
    ...
    locations = {
      "/" = {
        proxyPass =
          "http://unix:/srv/http/scoreboard-seedbox-cri/scoreboard-seedbox-cri.sock";
        extraConfig = ''
